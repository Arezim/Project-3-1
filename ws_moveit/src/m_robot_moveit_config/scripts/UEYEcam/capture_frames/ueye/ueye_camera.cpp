...
#include "ueye_camera.h"
      ...
      /*
      IplImage* tmpImg = cvCreateImageHeader (cvSize (MaxImageSizeX,MaxImageSizeY), IPL_DEPTH_8U, 1);
      tmpImg->imageData = pBuffer; //pcImageMemory;
      /*
      cvNamedWindow("src",1);
...
/*
void Ueye_camera::monitor(){
  const std::string fn=strfilename;
     cv::Mat tmpImg = cv::imread("/data/CoRoSect/10.code/ueye/c++/10.1.Parameters/images/frame_1.png", CV_LOAD_IMAGE_COLOR);
     //cv::namedWindow( "window", CV_WINDOW_AUTOSIZE );
       cv::imshow( "window", tmpImg ); 
       //cv::imwrite("image.jpg",tmpImg); 
       cv::waitKey(0);   
}*/
...
void Ueye_camera::capture_video(std::string savedir, bool save_frames){
  ...
  /*
  double frame_rate_enabled=1;
  nRet = is_SetAutoParameter(hCam,IS_SET_ENABLE_AUTO_FRAMERATE,&frame_rate_enabled,0);
  if (frame_rate_enabled){
    double dblFPS;
    nRet=is_GetFramesPerSecond (hCam,&dblFPS);
    std::cout<<"---------------------------------------------------------------Auto Frame rate is REE-ENABLED AND THE FPS values has been set to:"<<dblFPS<<std::endl;  
  }
  */
  ...
  //7. Acquire the images from camera in a loop:
    ...
    for (int j=0;j<(int) nNumberOfBuffers;++j){
      ...
      if (save_frames){
        ...
        //save_image();
        //output a single image form the camera
        //Save the image
...
void Ueye_camera::apply_automatic_focus(int afmin,int afmax){
  ...
  if (IS_SUCCESS == nRet)
  {
    ...
    //}
    //Just A PRINT to check ON HOW THIS THING WORKS...
    //(checked)std::cout<<"The sharpness algorithm set is :"<<sharpness_calculation_algorithm<<"\t AUTOFOCUS_SHARPNESS_CALCULATION_ALGORITHM_TENENGRAD: "<<AUTOFOCUS_SHARPNESS_CALCULATION_ALGORITHM_TENENGRAD<< "\t AUTOFOCUS_SHARPNESS_CALCULATION_ALGORITHM_MEAN_SCORE: "<<AUTOFOCUS_SHARPNESS_CALCULATION_ALGORITHM_MEAN_SCORE<<std::endl;
...
/*
void Ueye_camera::apply_automatic_image_control){// (1&2) Auto exposure shutter (AES) AND Auto gain control (AGC)
  ...
  double aes_enabled,agc_enabled, awb_enabled;
  //aes_enabled=agc_enabled=awb_enabled=1;
  aes_enabled=1;
  agc_enabled=1; 
  awb_enabled=1;

  nRet = is_SetAutoParameter (hCam, IS_GET_ENABLE_AUTO_SHUTTER, &aes_enabled, 0);
  method_report(nRet, "automatic_image_control::IS_GET_ENABLE_AUTO_SHUTTER");
  nRet = is_SetAutoParameter (hCam, IS_GET_ENABLE_AUTO_GAIN, &agc_enabled, 0);
  method_report(nRet, "automatic_image_control::IS_GET_ENABLE_AUTO_GAIN");
  nRet = is_SetAutoParameter (hCam, IS_GET_ENABLE_AUTO_WHITEBALANCE, &awb_enabled, 0);
  method_report(nRet, "automatic_image_control::IS_GET_ENABLE_AUTO_WHITEBALANCE");
  if (aes_enabled && agc_enabled && awb_enabled){
    std::cout<<"AES, AGC and AWB are enabled"<<std::endl; 
  }
  ...
  method_report(nRet, "automatic_image_control::IS_SET_AUTO_REFERENCE");
  double setpoint;
  ...
  method_report(nRet, "automatic_image_control::IS_SET_AUTO_GAIN_MAX");
  ...
  method_report(nRet, "automatic_image_control::IS_SET_AUTO_SHUTTER_MAX");
...
// (3) Auto white balance (AWB)
  ...
  //erasemeeeeeeeeeeeeeeeeeeeeeeeeeee --> not neccessary
  // Query minimum offset
  //double dOffsetRMin, dOffsetBMin;
  //nRet = is_SetAutoParameter (hCam, IS_GET_AUTO_WB_OFFSET_MIN, &dOffsetRMin, &dOffsetBMin);
  // Query maximum offset
  //double dOffsetRMax, dOffsetBMax;
  //nRet = is_SetAutoParameter (hCam, IS_GET_AUTO_WB_OFFSET_MAX, &dOffsetRMax, &dOffsetBMax);
  //std::cout<<std::endl<<dOffsetRMin << dOffsetBMin<< dOffsetRMax<<dOffsetBMax<<std::endl<<std::endl;;
  

//SKILLPOINT 2 : Speed and hysteresis
...
void Ueye_camera::auto_sexposure_gainc(){
  //IS_SET_ENABLE_AUTO_SENSOR_SHUTTER,IS_SET_ENABLE_AUTO_SENSOR_GAIN,IS_SET_ENABLE_AUTO_SENSOR_WHITEBALANCE  are not supported...
  //Enable auto exposure shutter(AES), auto gain control(AGC), white balance function(AWB)
  double dEnable = 1;
  nRet = is_SetAutoParameter (hCam, IS_SET_ENABLE_AUTO_SHUTTER, &dEnable, 0);
